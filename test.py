...
compensation_keywords = [
    ...
    "directors' compensation",        # regular apostrophe
    "director's compensation",        # regular apostrophe
    "directors' compensation",        # curly apostrophe (right single quotation mark)
    "director's compensation",        # curly apostrophe
    "directors&#8217; compensation",  # HTML entity for right single quotation mark
    "director&#8217;s compensation",  # HTML entity for right single quotation mark
    "directors&rsquo; compensation",  # HTML entity name for right single quotation mark
    "director&rsquo;s compensation",  # HTML entity name for right single quotation mark
]
...
def has_name_column(table):
    ...
    for row_idx, row in enumerate(rows[:5]):
        ...
        for idx, cell in enumerate(cells):
            # Get text content more carefully
            cell_text = extract_cell_text(cell).lower()
        ...
        for idx, cell in enumerate(cells):
            ...
            # But exclude columns that are clearly not person names
...
def extract_cell_text(cell):
    """Extract text from a cell, handling nested HTML and comments"""
    ...
    from bs4 import BeautifulSoup
    cleaned_cell = BeautifulSoup(cell_html, 'html.parser')
    
    # Extract text and clean it
    text = cleaned_cell.get_text()
...
def extract_names_from_table(table, name_column_index):
    ...
    # Keywords that indicate it's a job title/position, not a name
    title_keywords = [
        ...
        if len(cells) > name_column_index:
            ...
            if (name and len(name) > 2 and 
                ...
                if name and len(name) > 2:
                    ...
                    # Check if any title keyword appears in the name
                    has_title_keyword = any(keyword in name_lower for keyword in title_keywords)
                    
                    if has_title_keyword:
                        # Advanced check: try to strip the title part and keep just the name
                        # Find the earliest position where a keyword appears
                        ...
                        if earliest_pos > 0:
                            ...
                            # Remove trailing commas, dashes, or parentheses
                            clean_name = re.sub(r'[,\-—()\s]+$', '', clean_name).strip()
                            
                            # Check if the cleaned name is valid (at least 2 chars and looks like a name)
                            ...
                            # Keyword at the start, likely just a title
